...
from web_corp.pages.locators import LoginPageLocators

from global_variables import oauthurl

class webcorpLoginPage(BasePage):
    ...
    def should_be_login_url(self):
        assert oauthurl in self.browser.current_url, 'Login url is not found'
    ...
    def should_be_alert_text_about_wrong_data(self):
        assert self.__getattribute__(*LoginPageLocators.alert_about_wrong_data_text_locator), 'alert text present has been changed, should be "Неверно введен пароль. Проверьте правильность написания" '
